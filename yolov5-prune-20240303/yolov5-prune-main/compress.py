...
class HiddenPrints:  # 抑制程序中的所有打印输出
    def __enter__(self):
...
def get_pruner(opt, model, example_inputs):
    ...
    if opt.prune_method == "random":
        imp = tp.importance.RandomImportance()  # 生成随机的重要性分数来选择剪枝的参数
        # global_pruning: 是否进行全局剪枝，即跨多个层或整个网络选择剪枝的参数,MagnitudePruner: 通常基于参数的幅度进行剪枝，但结合随机重要性，它将随机选择剪枝参数。
        pruner_entry = partial(tp.pruner.MagnitudePruner, global_pruning=opt.global_pruning)
    elif opt.prune_method == "l1":
        # https://arxiv.org/abs/1608.08710
        imp = tp.importance.MagnitudeImportance(p=1)  # MagnitudeImportance(p=1): 使用L1范数（参数的绝对值）来确定参数的重要性。
        pruner_entry = partial(tp.pruner.MagnitudePruner, global_pruning=opt.global_pruning)
    elif opt.prune_method == "lamp":
        # https://arxiv.org/abs/2010.07611
        imp = tp.importance.LAMPImportance(p=2)  # LAMPImportance(p=2): LAMP重要性，这里的p=2可能指的是使用L2范数，虽然LAMP通常有更复杂的实现。
        pruner_entry = partial(tp.pruner.MagnitudePruner, global_pruning=opt.global_pruning)
    elif opt.prune_method == "slim":
        # https://arxiv.org/abs/1708.06519
        sparsity_learning = True
        imp = tp.importance.BNScaleImportance()   # BNScaleImportance: 使用批归一化层的缩放因子作为重要性度量。
        # BNScalePruner: 基于BN层的缩放因子进行剪枝。  reg: 正则化参数，用于控制剪枝的程度。
        ...
        # group_lasso: 是否使用组Lasso正则化，这有助于在组级别（例如卷积核）上施加稀疏性。
        ...
        imp = tp.importance.GroupNormImportance(p=2)  # GroupNormImportance(p=2): 使用组归一化的权重来评估重要性。
        # GroupNormPruner: 基于组归一化权重进行剪枝。
        ...
        sparsity_learning = True
        imp = tp.importance.GroupNormImportance(p=2)
        # GrowingRegPruner: 一个动态正则化增长的剪枝器。 delta_reg: 正则化增长的步长。
        ...
        # HessianImportance: 使用Hessian矩阵来评估参数重要性。 group_reduction: 在计算重要性时，组内参数如何聚合，这里是取均值。
        ...
        imp = tp.importance.TaylorImportance(group_reduction='mean')   # TaylorImportance: 使用Taylor展开式的一阶和二阶导数信息来评估参数的重要性。
        pruner_entry = partial(tp.pruner.MagnitudePruner, global_pruning=opt.global_pruning)
    else:
        raise NotImplementedError
    # 总结： 都是先找到评估哪些权重是否重要的衡量标准 然后使用剪枝器进行剪枝，reg来控制剪枝的程度
